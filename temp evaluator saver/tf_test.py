...
'''
def calculate_the_loss(test_y,pred_y):
    loss=0
    for i in range(len(test_y)):
        loss+=abs(test_y[i][0]-pred_y[i][0])
    loss=loss/len(test_y)
    return loss

def Neural_Networks_Nonlinear_Regression(train_x_set,train_y_set,test_x_set,test_y_set,bike_names,num_features,num_nodes_L1,num_nodes_L2,num_train,step_size):
    ...
    prediction = tf.nn.tanh(Wx_plus_b_L_Out)
...
#        best_loss=1
        ...
        for i in range(len(train_x_set)):
            ...
            for _ in range(num_train):
                ...
                #test_pred=sess.run(prediction,feed_dict = {x:x_test})
                #test_loss=calculate_the_loss(test_y_set,test_pred)
        ...
        #                break
            ...
            for j in range(len(y_data)):
                compare.append([y_data[j][0],y_pred[j][0],bike_names[j]])
